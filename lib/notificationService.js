...
/**
 * Notify eligible users when an approved job is updated with significant changes.
 * Sends "Job updated" message with jobId for deep link.
 */
async function createNotificationsForJobUpdate(job) {
  const jobObj = job.toObject ? job.toObject() : job;
  const jobId = jobObj._id?.toString?.() || jobObj._id;
  const title = 'Job updated';
  const body = (jobObj.title ? `"${jobObj.title}"` : 'A job') + ' has been updated. Tap to view.';
  ...
  for (const user of users) {
    ...
    if (pushOn && fcmTokens.length > 0) {
      try {
        ...
        console.error('[NotificationService] Job-update push error (non-fatal):', pushError.message);
      }
    }
    created++;
  }
...
function isSignificantJobChange(previous, updated) {
  const p = previous.toObject ? previous.toObject() : previous;
  const u = updated.toObject ? updated.toObject() : updated;
  const fields = ['title', 'lastDate', 'ageMin', 'ageMax', 'description', 'applicationLink', 'sourceLink'];
  for (const f of fields) {
    const pv = p[f];
    const uv = u[f];
    if (JSON.stringify(pv) !== JSON.stringify(uv)) return true;
  }
  if (JSON.stringify(p.dates) !== JSON.stringify(u.dates)) return true;
  if (JSON.stringify(p.eligibleStates) !== JSON.stringify(u.eligibleStates)) return true;
  if (JSON.stringify(p.vacancySeats) !== JSON.stringify(u.vacancySeats)) return true;
  if (JSON.stringify(p.eligibilityRules) !== JSON.stringify(u.eligibilityRules)) return true;
  return false;
}
